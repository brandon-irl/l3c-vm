...
// 65,536 memory locations each of which stores a 16-bit value
...
int main(int argc, const char *argv[])
{
	...
	while (running)
	{
		...
		case OP_JSR:
		{
			...
			if ((instr >> 11) & 0x1)
			{
				reg[PC] += sign_extend(instr & 0x7FF, 11);
			}
			else
			{
				reg[PC] = reg[(instr >> 6) & 0x7];
			}
		}
		break;
		case OP_LD:
		...
		case OP_LDI:
		{
			...
			// PCoffset 9
			uint16_t pc_offset = sign_extend(instr & 0x1FF, 9);
			// add pc_offset to the current pc, loot at that memory location to get the final address
